...
class Camera:
    ...
    async def _run_analysis(
        ...
        if full_frame_b64 is not None:
            task_items.append(("full_frame", self.analysis_client.query_description_open([full_frame_b64])))
        if selection_1_images:
            task_items.append(("uniform", self.analysis_client.query_description_open(selection_1_images)))
        if selection_2_images:
            task_items.append(("varied", self.analysis_client.query_description_open(selection_2_images)))

        results = await asyncio.gather(*(task for _, task in task_items))
        return {name: result for (name, _), result in zip(task_items, results)}
